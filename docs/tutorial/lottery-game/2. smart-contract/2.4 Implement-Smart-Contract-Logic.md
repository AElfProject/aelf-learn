...
In this step, we will be implementing the logic of our Lottery Game smart contract in the **src/LotteryGame.cs** file!
...
You might have noticed that some functions (e.g. Play) has its function body left relatively empty with a TODO. We are going to implement them below.
...
```
Line 11 shows how you can reference to any contract using the contract address. In this case, we have initialized `TokenContract` and we would then be able to use it to reference the Multitoken Contract in other functions in order to call methods.

:::tip
You may reference the name of any System Contracts by doing this:  
`State.TokenContract.Value = Context.GetContractAddressByName(SmartContractConstants.TokenContractSystemName);`
:::

## Step 3: Calling Contract Methods through Contract Reference State

We are going to make use of `TokenContract` reference State in Play.  
The logic of Play is as below:
1. Check that the amount played by the player does not exceed our limits.
...
You may choose to implement the rest of the functions as a practice, but we have provided a complete implementation below so that you can deploy and play with your Lottery Game Contract.
